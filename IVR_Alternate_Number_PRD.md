...
**Boundary.** This spec governs **CSP-initiated** IVR calls on Install, Service (restore) and Pickup tickets — every ticket family the IVR serves — at every CSP where IVR 2.0 is live. It covers two things: **holding** a customer's alternate numbers, and **using** them on a call. It leaves unchanged:

- **Customer-initiated calls** — the escalation chain from the executor to the CSP's manager and owner is a separate PRD (AC-REG-1).
...
- **Bridge-rate behaviour** — caller identification, PIN entry and dead-end handling are governed by the existing IVR product spec (AC-REG-3).
...
|---|---|---|---|---|
| C-01 | Maximum numbers dialled in one fallback run, registered number included (T1, R4) | 3 | 1–5 ⚠️ *AI GENERATED — review* | Product |
| C-02 | How long an alternate stays usable after it was last seen (N5, R6) | 6 months | 1–24 months ⚠️ *AI GENERATED — review* | Product |
| C-03 | Fallback enabled — kill switch (T1) | On, wherever IVR 2.0 is live | On / Off | Product + Eng ⚠️ *AI GENERATED — review* |
