...
|---|---|---|---|---|---|
| T1 | — | CSP-initiated call resolved to a customer and an in-scope ticket | Fallback enabled (C-02); the store returns at least one alternate for that customer | Ringing position 1 | Registered number dialled first (R2, G2). The dial list is resolved and frozen now: registered number, then the alternates the store returned, in its order, capped at C-01 (R3). List length recorded (MQ-4). |
